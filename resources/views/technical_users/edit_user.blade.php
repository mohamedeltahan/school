        ...
        <div class="row text-right">
            ...
            <div class="form-container col-md-6">
                <form method="post" action="{{route('technical_users.update',$technical_user->id)}}">
                    @method("PUT")
                    @csrf
                    <legend>تعديل مستخدم <b>{{ $technical_user->full_name }}</b></legend>
                    ...
                    <div class="form-group investors-optional investors-options">
                        ...
                        <select name="investor_id" class="form-control  text-right fast-search-select" data-search-target=".schools-container" data-target-coulmn="inverstor" data-target="#schools-container"  data-type="table" data-name="investor">
                            @foreach($supports as $support)
                                <option @if($technical_user->investor_id==$support->id) {{'selected=true'}} @endif  value="{{$support->id}}">{{$support->full_name}}</option>
                            @endforeach
                        </select>
                    </div><!-- /.form-group -->
                    ...
                    <div class="form-group privliges-table">
                        ...
                        <table class="table table-striped table-condensed">
                            ...
                            <tr>
                                <td class="text-right">
                                    <input value="{{$permission->name}}" name="permissions[]" @if(\App\User::where("user_id",$technical_user->id)->first()->can($permission->name)) checked @endif  type="checkbox">
                                </td>
                                <td class="text-right">
                                   {{ $permission->name}}
                                </td>
