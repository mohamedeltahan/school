...
<div class="container-fluid">
  ...
  <div class="row add-account-btn">
    <div class="col-md-12">
      <a href='http://localhost/school/public/students/create' class="btn btn-primary pull-left">اضافة طالب جديد</a>
    </div><!-- /.col-md-12 -->
  </div><!-- /.row -->
      ...
      <div class="users-control-table">
        <table class="table table-striped table-hover">
          ...
          @foreach($students as $student)
            <tr>
              <td class="text-center"><a href="{{route('students.show', $student->id)}}">{{ $student->id }}</a></td>
              <td class="text-center"><a href="{{route('students.show', $student->id)}}">{{ $student->full_name }}</a></td>
              <td class="text-center">{{ $student->level }}</td>
              <td class="text-center"> {{$schools[$student->id]->Adminstration}} </td>
              ...
              <td class="text-center">
                ...
                <form style="display:inline;" method="post" action="{{route("students.destroy",$student->id)}}">
                    @method("delete")
                    @csrf
                    <button class="btn btn-danger btn-xs confirm-delete" data-confirm-name="{{$student->name}}" type="submit">حذف</button>
                </form>
              </td>
            </tr>
          @endforeach
