      ...
      <div class="panel-body">
        ...
        <div class="users-control-table">
		  ...
		  @foreach($technical_users as $user)
						 <tr>
							 <td class="text-right"><a href='{{route('technical_users.show',$user->id)}}'>{{$user->id}}</a></td>
							 <td class="text-right"><a href='{{route('technical_users.show',$user->id)}}'>{{$user->full_name}}</a></td>
                 ...
                 <form style="display:inline;" method="post" action="{{route("technical_users.destroy", $user->id)}}">
                     @method("delete")
                     @csrf
                     <button class="btn btn-danger btn-xs confirm-delete" data-confirm-name="{{$user->name}}" type="submit">حذف</button>
                 </form>
							 </td>
						 </tr>
            @endforeach
